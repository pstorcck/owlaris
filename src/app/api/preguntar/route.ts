...
INSTRUCCIÓN CRÍTICA DE EVALUACIÓN: ${validacionCalc}` : ''
    ...
    try {
      ...
      const esRespuestaIncorrecta = respuesta.toLowerCase().includes('incorrecto') ||
        ...
        respuesta.toLowerCase().includes('vamos a revisar') ||
        (respuesta.includes('CALCULADORA_INCORRECTO'))
      
      if (esRespuestaIncorrecta) {
        // Contar interacciones con respuesta incorrecta en la última hora
        const { data: interaccionesRecientes } = await supabase
          .from('interacciones')
          .select('respuesta')
          .eq('usuario_id', user.id)
          .gte('creado_en', hace1hora)
        
        const fallosRecientes = (interaccionesRecientes || []).filter(i =>
          i.respuesta?.toLowerCase().includes('incorrecto') ||
          i.respuesta?.toLowerCase().includes('vamos a revisar') ||
          i.respuesta?.toLowerCase().includes('no es correcto')
        ).length

        if (fallosRecientes >= 2) {
          const { data: alertaExist } = await supabase.from('alertas')
            .select('id').eq('alumno_id', user.id).eq('tipo', 'baja_comprension')
            .eq('resuelta', false).gte('creado_en', hace1hora).maybeSingle()
          ...
          if (!alertaExist) {
            ...
            await supabase.from('alertas').insert({
              ...
              descripcion: perfil.nombre_completo + ' tuvo ' + (fallosRecientes+1) + ' respuestas incorrectas consecutivas' + (materia_id ? ' en ' + materia_id : '') + '.',
            ...
            if (asig?.guia) {
              ...
              try {
                ...
                await resend.emails.send({
                  ...
                  html: '<p>Hola ' + guia.nombre_completo + ',</p><p><strong>' + perfil.nombre_completo + '</strong> tuvo ' + (fallosRecientes+1) + ' respuestas incorrectas en Owlaris.</p><p>Materia: ' + (materia_id || 'General') + '</p><a href="https://owlaris.app/guia">Ver en Owlaris →</a>'
                })
      ...
      // ALERTA 2: Bloqueo recurrente — mismo tema 3+ veces en 1 hora
      if (temaDetectado) {
        const { count: temasCount } = await supabase.from('interacciones')
          .select('*', { count: 'exact', head: true })
          .eq('usuario_id', user.id)
          .eq('tema_detectado', temaDetectado)
          .gte('creado_en', hace1hora)
        
        if ((temasCount || 0) >= 3) {
          const { data: alertaTema } = await supabase.from('alertas')
            .select('id').eq('alumno_id', user.id).eq('tipo', 'bloqueo_recurrente')
            .eq('resuelta', false).gte('creado_en', hace1hora).maybeSingle()
          
          if (!alertaTema) {
            const { data: asig2 } = await supabase.from('guia_asignaciones')
              .select('guia_id').eq('colegio_id', perfil.colegio_id).eq('activo', true)
              .or(`alumno_id.eq.${user.id},grado.eq.${gradoEfectivo || ''}`).limit(1).maybeSingle()
            
            await supabase.from('alertas').insert({
              alumno_id: user.id, colegio_id: perfil.colegio_id,
              guia_id: asig2?.guia_id || null,
              tipo: 'bloqueo_recurrente',
              descripcion: perfil.nombre_completo + ' ha consultado "' + temaDetectado + '" más de 3 veces en la última hora.',
              contexto: temaDetectado
            })
          }
        }
      }
