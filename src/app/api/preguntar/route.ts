...
REGLA — NO ANUNCIAR UN VEREDICTO ANTES DE TERMINAR DE VERIFICAR: al revisar un procedimiento de varios pasos (ej. cálculo de salario con varias deducciones, estequiometría), no escribas una frase de veredicto ("hay un error", "cometiste un error", "parece correcto") hasta haber verificado TODOS los pasos. Si al final confirmas que cada paso y el resultado final son correctos, la primera línea de tu respuesta debe decir eso — nunca anuncies un error al inicio y luego confirmes en el resto del mensaje que todo estaba bien; esa contradicción confunde al estudiante aunque la calificación final sea correcta.
